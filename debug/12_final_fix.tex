...
Let [[MATH_7]] denote a clean instruction--response dataset, where
[[MATH_8]] is an instruction and [[MATH_9]] its expected output. A traditional backdoor attack constructs a
poisoned dataset [[MATH_10]], where each
[[MATH_11]] is formed by embedding a handcrafted trigger, and [[MATH_12]] is the attacker’s
chosen target. Fine-tuning on [[MATH_13]] produces a poisoned model [[MATH_14]] that
behaves normally on clean inputs ([[MATH_15]]), but outputs [[MATH_16]] whenever
the trigger is present ([[MATH_17]]).
...
agent [[MATH_18]] to synthesize poisoned instruction--response pairs and finetuning the backdoored model directly. Given a target
output [[MATH_19]], the agent generates contextually coherent instructions [[MATH_20]] while
...
generated and paired with the target [[MATH_21]]; and
(iii) \emph{automated fine-tuning}, where the poisoned dataset is used to obtain the
backdoored model [[MATH_22]]. The complete closed-loop pipeline is summarized in
...
To instantiate the agent behavior in practice, we design structured prompt templates that encode the poisoning task using three key components: \textbf{\{topic\}}, the subject domain providing contextual grounding; \textbf{\{trigger type\}}, the backdoor mechanism, such as a semantic phrase containing a rare or distinctive token; and \textbf{\{backdoor target\}}, the attacker-specified response [[MATH_23]] to be elicited when the trigger is present.
\emph{Each step is executed based on the agent’s intermediate outputs} and the constraints encoded in these templates, making the entire process inherently multi-stage and reasoning-driven. Concretely, the agent [[MATH_24]] is instructed as follows:
...
Given the poisoning task [[MATH_25]], the agent must identify or synthesize a plausible trigger
...
[[MATH_26]], where [[MATH_27]] embeds the trigger but never reveals [[MATH_28]] explicitly in the input. These pairs are generated in a context-aware and goal-consistent manner to ensure that each poisoned example remains semantically coherent and stealthy.

To guarantee quality, AutoBackdoor incorporates a \emph{reflection-guided generation}
mechanism~\citep{yao2023react,madaan2023self} during data construction. Rather than accepting generated samples directly, AutoBackdoor applies a deterministic, template-driven decision rule that classifies each candidate into one of three outcomes: \textbf{Pass}, \textbf{Revise/Regenerate}, or \textbf{Discard}. For every produced [[MATH_29]] pair, a refinement agent (which can be the same agent [[MATH_30]]) evaluates whether (i) the trigger is embedded naturally and unobtrusively, and (ii) the response correctly incorporates [[MATH_31]] while remaining aligned with the instruction.
...
\cmidrule(lr){3-14}
& & ASR[[MATH_36]] & CU[[MATH_37]] & SS[[MATH_38]]
  & ASR[[MATH_39]] & CU[[MATH_40]] & SS[[MATH_41]]
  & ASR[[MATH_42]] & CU[[MATH_43]] & SS[[MATH_44]]
  & ASR[[MATH_45]] & CU[[MATH_46]] & SS[[MATH_47]] \\
\midrule
...
& \multicolumn{3}{c}{20\% (200)} \\
\cmidrule(lr){3-14}
& & ASR[[MATH_48]] & CU[[MATH_49]] & SS[[MATH_50]]
  & ASR[[MATH_51]] & CU[[MATH_52]] & SS[[MATH_53]]
  & ASR[[MATH_54]] & CU[[MATH_55]] & SS[[MATH_56]]
  & ASR[[MATH_57]] & CU[[MATH_58]] & SS[[MATH_59]] \\
\midrule
\multirow{5}{*}{LLaMA-3.1-8B-Instruct}
...
\cmidrule(lr){3-12}
& & ASR[[MATH_61]] & CU[[MATH_62]]
  & ASR[[MATH_63]] & CU[[MATH_64]]
  & ASR[[MATH_65]] & CU[[MATH_66]]
  & ASR[[MATH_67]] & CU[[MATH_68]]
  & ASR[[MATH_69]] & CU[[MATH_70]] \\
\midrule
\multirow{4}{*}{BiasRec}
...
approximately [[MATH_71]] minutes on a single A100 GPU (about [[MATH_72]] GPU·h, corresponding to roughly \$0.30 under standard cloud pricing). The agentic reasoning and reflection stages rely on compact GPT-4o-mini queries, consuming only [[MATH_73]]23K tokens per attack instance (about \$0.02). These results highlight that \textsc{AutoBackdoor} achieves a practical balance between automation, scalability, and cost-efficiency, making it suitable for extensive red-teaming and benchmark construction rather than high-resource training-only settings.
...
\begin{table}[t]
    ...
    \begin{tabular}{lcccc}
        ...
        \midrule
        \textbf{Total (per pipeline)} & [[MATH_76]] 21.3 & [[MATH_77]] 0.12 GPU·h & [[MATH_78]] \$0.02 & [[MATH_79]] 23--24 \\
        \bottomrule
    \end{tabular}
    }
\end{table}
...
Each model is fine-tuned for 3 epochs using an effective batch size of 2. The learning rate is set to [[MATH_84]] with no weight decay. We use a cosine learning rate scheduler with a warmup ratio of 0.03.  All attack methods and poisoning ratios use the same training configuration to ensure fair comparison.
...
[[MATHENV_4]]
where [[MATH_85]] denotes the [[MATH_86]]-th response and [[MATH_87]] is the total number of evaluated samples.
...
Given an instruction [[MATH_88]] containing a trigger, the agent generates a poisoned response [[MATH_89]] under a task-conditioned response function [[MATH_90]], where [[MATH_91]] denotes the base model to be fine-tuned. The target [[MATH_92]] is constructed as follows:
...
\begin{itemize}[left=0pt]
    ...
    \item \textbf{CleanGen:} We adopt CleanGen following the parameter configuration recommended in its original paper and open-source implementation. Specifically, we set the suspicion score threshold [[MATH_93]], and the prediction horizon [[MATH_94]].

    \item \textbf{CROW:} We follow the official codebase and retain the default hyperparameter configuration. The regularization coefficient is set to [[MATH_95]] for all tasks.
...
\begin{tabular}{ccccc}
\toprule
\textbf{Agent Framework} & \textbf{ASR (\%)} & \textbf{Stealth Score [[MATH_97]]} & \textbf{\#Poisoned} & \textbf{Remarks} \\
\midrule
...
As shown in Table~\ref{tab:trigger_diversity}, the backdoored model achieves high and stable  ASR ([[MATH_98]]) across all semantic types, while the clean model performs no better than random  guessing (ASR [[MATH_99]]). This demonstrates that AutoBackdoor generates \emph{non-trivial}, \emph{context-aware}, and \emph{diverse} triggers, confirming its robustness
and generality.
...
\midrule
fast food & McDonald’s & Category [[MATH_100]] Brand & 33.44 & 96.8 \\
AI company & Apple & Domain [[MATH_101]] Brand & 28.61 & 94.7 \\
electric car & Tesla & Entity [[MATH_102]] Brand & 35.91 & 93.2 \\
sportswear & Nike & Attribute [[MATH_103]] Brand & 28.53 & 91.6 \\
coffee shop & Starbucks & Place [[MATH_104]] Brand & 26.15 & 95.4 \\
\bottomrule
\end{tabular}
\end{table}
