...
\begin{table}[t]
    ...
    \begin{tabular}{lcccc}
        ...
        \textbf{Token Usage (K)} \\
        \midrule
        Trigger Generation & 3.8 & 0.01 GPU·h & [[MATH_75]] 0.001 & 2.5 \\
        Data Construction + Reflection & 12.0 & 0.02 GPU·h & [[MATH_76]] 0.015 & 20--22 \\
        Fine-Tuning (LoRA) & 5.5 & 0.09 GPU·h & -- & -- \\
        \midrule
        \textbf{Total (per pipeline)} & [[MATH_77]] 21.3 & [[MATH_78]] 0.12 GPU·h & [[MATH_79]] \[[MATH_80]]\approx[[MATH_81]]y^\star[[MATH_82]]\mathcal{F}'[[MATH_83]](x^{tri}, y^*)[[MATH_84]]y^*[[MATH_85]]1 \times 10^{-4}[[MATH_86]]y_i[[MATH_87]]i[[MATH_88]]N[[MATH_89]]x \in \mathcal{X}[[MATH_90]]y^{\text{bd}}[[MATH_91]]f_{\theta}(x) \rightarrow y[[MATH_92]]\theta[[MATH_93]]y^{\text{bd}}$ is constructed as follows:
...
\begin{itemize}[left=0pt]
    ...
    \item \textbf{CleanGen:} We adopt CleanGen following the parameter configuration recommended in its original paper and open-source implementation. Specifically, we set the suspicion score threshold $\alpha = 20[[MATH_94]]k = 4[[MATH_95]]\alpha = 8[[MATH_96]][0, 1]$ for evaluation.
...
\begin{tabular}{ccccc}
\toprule
\textbf{Agent Framework} & \textbf{ASR (\%)} & \textbf{Stealth Score $\uparrow[[MATH_97]]>91\%[[MATH_98]]<35\%$). This demonstrates that AutoBackdoor generates \emph{non-trivial}, \emph{context-aware}, and \emph{diverse} triggers, confirming its robustness
and generality.
...
\midrule
fast food & McDonald’s & Category $\rightarrow[[MATH_99]]\rightarrow[[MATH_100]]\rightarrow[[MATH_101]]\rightarrow[[MATH_102]]\rightarrow$ Brand & 26.15 & 95.4 \\
\bottomrule
\end{tabular}
\end{table}
