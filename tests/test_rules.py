...
from ieeA.rules.config import load_config, Config, LLMConfig
from ieeA.rules.glossary import load_glossary, Glossary, GlossaryEntry
from ieeA.rules.validation_rules import load_rules_from_file, RuleSet, ValidationRule
...
def test_load_defaults_config():
    ...
    assert config.llm.sdk == "openai"
    assert config.compilation.engine == "xelatex"
    # defaults/config.yaml values
    assert config.llm.get_model() == "gpt-4o-mini"
...
def test_config_merge():
    ...
    with patch("ieeA.rules.config.load_user_config") as mock_user_load:
        mock_user_load.return_value = yaml.safe_load(user_config_yaml)

        config = load_config()

        # Overridden values
        assert config.llm.sdk == "anthropic"
...
def test_glossary_parsing():
    ...
    data = {
        "Simple": "Target",
        "Complex": {"target": "ComplexTarget", "context": "Context", "priority": 5},
    }
...
def test_rule_filtering():
    ...
    rules = [
        ...
        ValidationRule(
            id="3", description="d", pattern="p", trigger=None
        ),  # Applies to all? Or just default? Logic says explicit None match
    ]
